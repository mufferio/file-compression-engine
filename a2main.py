...
class Compressor:
    ...
    def compress(pixels: List[List[int]], loss_level: int) -> QuadTree:
        ...
        print("raw-pixel range:", min(flat), "→", max(flat))
...
class Decompressor:
    ...
    def run(self) -> None:
        ...
        bmp_file.set_data(
            ...
            body=pixels_triple)
        flat = [v for row in pixels_single for v in row]
        print("decompressed range:", min(flat), "→", max(flat))
        bmp_file.save()
...
if __name__ == '__main__':
    print("<==> QUAD TREE IMAGE COMPRESSOR <==>")
    print("input 'q' at any time to quit the app")
    comp_decomp = input("Command [c -> Compress | d -> Decompress]: ")
    if comp_decomp == "c":
        loss = input("Loss [between 0-255]: ")
        file = input("File Name (.bmp): ")
        print(f"Compression is starting on {file}")
        k = Compressor(file, int(loss))
        k.run()
        print(f"file saved as {file}.qdt")
        print("Compression Done")
    elif comp_decomp == 'd':
        qdt_name = input("File Name (.qdt): ")
        print(f"Decompression is starting on {qdt_name}")
        dec = Decompressor(qdt_name)
        dec.run()
        out_bmp = qdt_name[:-4] + BMP_EXTENSION
        print(f"file saved as {out_bmp}")
